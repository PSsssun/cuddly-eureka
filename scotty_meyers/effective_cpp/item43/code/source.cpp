...
public:
    ...
    inline std::string get_company_name() const {return company_name_;}
...
void MsgSender<Company>::SendClear(const MsgInfo& info) {
    std::string msg = info.get_company_name() + " " + info.get_message() + " " + info.get_time();
    Company c;
    c.SendCleartext(msg);
}
...
int main() {
    ...
    MsgInfo info("Google", "Give you an offer!", "Henry");
